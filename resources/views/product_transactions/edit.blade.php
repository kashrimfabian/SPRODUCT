...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <h4 class="text-center mb-4">Edit Product Transaction</h4>
            ...
            <div class="card">
                <div class="card-header">Edit Transaction Record (ID: {{ $product_transaction->trans_id }})</div> {{-- Updated --}}
                <div class="card-body">
                    <form action="{{ route('product_transactions.update', $product_transaction->trans_id) }}" method="POST"> {{-- Updated --}}
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="cust_ali_id" name="cust_ali_id" required>
                                ...
                                <option value="{{ $batch->cust_ali_id }}" 
                                    {{ old('cust_ali_id', $product_transaction->cust_ali_id) == $batch->cust_ali_id ? 'selected' : '' }}
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="user_id" name="user_id" required>
                                ...
                                <option value="{{ $user->id }}" {{ old('user_id', $product_transaction->user_id) == $user->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" id="tarehe" name="tarehe" {{-- Updated --}}
                                value="{{ old('tarehe', $product_transaction->tarehe) }}" required> {{-- Updated --}}
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="trans_type" name="trans_type" required> {{-- Updated --}}
                                ...
                                <option value="collection" {{ old('trans_type', $product_transaction->trans_type) == 'collection' ? 'selected' : '' }}>Customer Collection</option> {{-- Updated --}}
                                <option value="sale" {{ old('trans_type', $product_transaction->trans_type) == 'sale' ? 'selected' : '' }}>Factory Sale (on customer's behalf)</option> {{-- Updated --}}
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="product_id" name="product_id" required>
                                ...
                                <option value="{{ $product->product_id }}" {{-- Using product_id as value --}}
                                    {{ old('product_id', $product_transaction->product_id) == $product->product_id ? 'selected' : '' }}
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="quantity" name="quantity"
                                value="{{ old('quantity', $product_transaction->quantity) }}" min="0.01" required>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="amount" name="amount"
                                value="{{ old('amount', $product_transaction->amount) }}" min="0">
                        ...
                        <div class="mb-3" id="buyer_name_field">
                            <label for="buyer_name" class="form-label">Buyer Name (For Sales)</label>
                            <input type="text" class="form-control" id="buyer_name" name="buyer_name"
                                value="{{ old('buyer_name', $product_transaction->buyer_name) }}" maxlength="255">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Mapping for stock display based on product names in the customer stock dropdown
        ...
        function toggleTransactionFields() {
            if (transactionTypeSelect.value === 'sale') {
                ...
                amountField.setAttribute('required', 'required'); // Amount is required for sales
                ...
                amountField.removeAttribute('required'); // Amount is optional for collection
                document.getElementById('buyer_name').value = ''; // Clear buyer name if switching to collection
            } else {
        ...
        // Set initial state based on current value when editing
        const initialTransactionType = transactionTypeSelect.value;
        if (initialTransactionType === 'sale') {
            buyerNameField.style.display = 'block';
        ...
        // toggleTransactionFields() is handled by the initial setup above for edit views
        updateProductInfo();
    });
</script>
@endsection