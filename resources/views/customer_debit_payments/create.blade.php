...
<div class="container">
    <div class="card shadow-lg rounded-3" >
        <div class="card-header bg-primary ">
            <h4 class="mb-0">Add Customer Debit Payment</h4>
        </div>
        <div class="card-body p-4">
            ...
            <form action="{{ route('customer_debit_payments.store') }}" method="POST">
                @csrf
                    ...
                    <select name="debit_id" id="debit_id" class="form-select rounded-pill @error('debit_id') is-invalid @enderror" required>
                        ...
                        @foreach($debits as $debit)
                            <option value="{{ $debit->debit_id }}"
                                ...
                                {{ old('debit_id') == $debit->debit_id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="payment_date" id="payment_date" class="form-control rounded-pill @error('payment_date') is-invalid @enderror" value="{{ old('payment_date', date('Y-m-d')) }}" required>
                    @error('payment_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" name="amount" id="amount" class="form-control rounded-pill @error('amount') is-invalid @enderror" value="{{ old('amount') }}" required>
                    @error('amount')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="notes" id="notes" class="form-control rounded-3 @error('notes') is-invalid @enderror" rows="3">{{ old('notes') }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
...
<script>
    $(document).ready(function() {
        ...
        // Call on page load to set initial balance if an option is pre-selected (e.g., old() value)
