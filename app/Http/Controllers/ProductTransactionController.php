...
class ProductTransactionController extends Controller
{
    ...
    {
        $query = ProductTransaction::with(['product', 'custAlizeti.customer', 'user']);
        ...
        if ($request->filled('buyer_name')) {
            $query->where('buyer_name', 'like', '%' . $request->input('buyer_name') . '%');
        }
        ...
        $productTransactions = $query->paginate(10); 

        $products = Product::whereIn('name', array_keys($this->productToStockColumnMap))->orderBy('name')->get(); 

        $users = User::orderBy('first_name')->get();   

        
        return view('product_transactions.index', compact(
            'productTransactions', 
            'products',
            'users'                
        ));
    ...
    public function confirm(ProductTransaction $product_transaction)
    {
        ...
        try {
            ...
            if (!$stockColumn) {
                throw new \Exception('Product mapping missing for "' . $productName . '". Cannot confirm transaction.');
            }

            $customerStock = $product_transaction->custAlizeti->customerStock;

            if (!$customerStock) {
                throw new \Exception('Customer stock record not found for this batch. Cannot confirm transaction.');
            }


            if ($product_transaction->trans_type === 'sale') {
                
                if ($customerStock->$stockColumn < $product_transaction->quantity) {
                    throw new \Exception('Insufficient ' . $productName . ' in customer\'s batch (' . $product_transaction->custAlizeti->batch_no . ') for sale confirmation. Available: ' . number_format($customerStock->$stockColumn, 2) . ' ' . $product_transaction->product->unit_of_measure . '.');
                }
                $customerStock->$stockColumn -= $product_transaction->quantity;
            } elseif ($product_transaction->trans_type === 'collection') {
               
                $customerStock->$stockColumn += $product_transaction->quantity; 
            } else {
                throw new \Exception('Unsupported transaction type for confirmation: ' . $product_transaction->trans_type);
            }
    ...
    public function destroy(ProductTransaction $product_transaction)
    {
        ...
        try {
            ...
            if ($product_transaction->status === 'confirmed') {
                ...
                if (!$stockColumn) {
                    throw new \Exception('Product mapping missing for "' . $productName . '" during deletion reversal. Cannot revert stock.');
                }
                ...
                if ($customerStock) {                    
                    if ($product_transaction->trans_type === 'sale') {                        
                        ...
                        $customerStock->$stockColumn -= $product_transaction->quantity;                       
                        if ($customerStock->$stockColumn < 0) {
                             Log::warning("Attempted to reverse collection for product '{$productName}' for batch '{$product_transaction->custAlizeti->batch_no}' resulting in negative stock. Setting to 0. Transaction ID: {$product_transaction->trans_id}");
                            $customerStock->$stockColumn = 0; 
                        }
                    } else {

                        throw new \Exception("Unsupported transaction type '{$product_transaction->trans_type}' for stock reversal during deletion. Please review controller logic.");
                    }
                    
                    $customerStock->save(); 
                } else {
                    throw new \Exception("Customer stock record for batch ID {$product_transaction->cust_ali_id} missing. Cannot revert stock during deletion.");
                }
            }            
